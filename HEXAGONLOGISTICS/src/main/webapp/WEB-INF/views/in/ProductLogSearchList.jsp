...
<a href="/in/ProductLogSearchList?select_num=2&pageNum=${product.startPage - 5}&code=${param.code}">[以前]</a>
</c:if>
<c:forEach var="pNo"
begin="${product.startPage}"
end="${product.endPage}">
<a href="/in/ProductLogSearchList?select_num=2&pageNum=${pNo}&code=${param.code} ">[${pNo}]</a>
</c:forEach>
<c:if test="${productLogPage.endPage < product.totalPages}">
<a href="/in/ProductLogSearchList?select_num=2&pageNum=${product.startPage + 5}&code=${param.code}">[次のページ]</a>
</c:if>
</td>
</tr>
<%-- </c:if> --%>
